...
from app.schemas.user import ClerkWebhookEvent, WebhookResponse
...
async def handle_clerk_webhooks(
    request: Request, db: Annotated[DatabaseSessionManager, Depends(get_db_session_manager)]
) -> WebhookResponse:
    ...
    Args:
        ...
        db: Database session manager injected by FastAPI
    ...
    clerk_user_sync_service = ClerkUserSyncService(db)
    ...
    try:
        ...
        if webhook_data.type == "user.created":
            ...
            await clerk_user_sync_service.sync_user_updates(webhook_data)
            return WebhookResponse(status="updated")

        elif webhook_data.type == "user.deleted":
            await clerk_user_sync_service.sync_user_deletion(webhook_data.data.id)
